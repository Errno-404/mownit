...
def plot_spline(x, y, x_interp, y_interp):
    plt.plot(x_interp, fun(x_interp), label='f(x)')
...
def calculate_everything(nodes_type, n):
    ...
    b, c, d = calculate_cubic_spline(nodes, fun(nodes))
    y_axis = evaluate_spline(x_axis, nodes, fun(nodes), b, c, d)

    plot_spline(nodes, fun(nodes), x_axis, y_axis)
...
# brakuje jeszcze splice2 no i maybe trochę teorii, żeby nie siedzieć jak widły w gnoju XD